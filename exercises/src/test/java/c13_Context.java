...
public class c13_Context extends ContextBase {
    ...
    public void pagination() {
        ...
        Flux<Integer> results = Mono.deferContextual(ctx -> {
                    ...
                    if (signal.isOnNext()) {
                        (pageNr).incrementAndGet();
                    } else if (signal.isOnError()) {
                        pageWithError.set(pageNr.getAndIncrement());
                        System.out.println("Error at page: " + pageWithError.get());
                        System.out.println("Error:" + signal.getThrowable().getMessage());
                    }
                })
