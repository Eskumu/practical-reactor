...
public class c6_CombiningPublishers extends CombiningPublishersBase {
    ...
    public void mail_box_switcher() {
        Flux<Message> myMail = mailBoxPrimary()
                        .switchOnFirst(((firstSignal, originalMailBox) -> {
                    if (firstSignal.hasValue() && firstSignal.get().metaData.equals("spam")) {
    ...
    public void acid_durability() {
        Flux<String> committedTasksIds = tasksToExecute()
                .concatMap(task ->
                        task.flatMap(taskId ->
                                commitTask(taskId).thenReturn(taskId)));
    ...
    public void cleanup() {
        ...
        Flux<String> stream = Flux.usingWhen(
                StreamingConnection.startStreaming(), // opens the stream
                n -> n, // modifies stream (result is exposed)
                complete -> StreamingConnection.closeConnection() // action to close it in async way
        );
